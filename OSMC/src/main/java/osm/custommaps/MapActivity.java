...
public class MapActivity extends Activity implements MapEventsReceiver, LocationListener, SensorEventListener {
	...
	@Override public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(osm.custommaps.R.layout.main);
		...
		map = (MapView) findViewById(osm.custommaps.R.id.map);
		GenericMapView genericMap = (GenericMapView) findViewById(osm.custommaps.R.id.map);
		...
		mItineraryMarkers.setName(getString(osm.custommaps.R.string.itinerary_markers_title));
		map.getOverlays().add(mItineraryMarkers);
		mViaPointInfoWindow = new ViaPointInfoWindow(osm.custommaps.R.layout.itinerary_bubble, map);
		updateUIWithItineraryMarkers();

		//Tracking system:
		mTrackingModeButton = (Button)findViewById(osm.custommaps.R.id.buttonTrackingMode);
		...
		/*Button searchDepButton = (Button)findViewById(osm.custommaps.R.id.buttonSearchDep);
	...
	/*	AutoCompleteOnPreferences destinationText = (AutoCompleteOnPreferences) findViewById(osm.custommaps.R.id.editDestination);
		...
		searchDestButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View view) {
				handleSearchButton(R.id.editDestination);
			}
		});
*/
		...
		Drawable clusterIconD = getResources().getDrawable(osm.custommaps.R.drawable.marker_poi_cluster);
	...
	public void addViaPoint(GeoPoint p){
		...
		updateItineraryMarker(null, p, viaPoints.size() - 1,
				osm.custommaps.R.string.viapoint, osm.custommaps.R.drawable.marker_via, -1, null);
	}
	...
	public void updateUIWithItineraryMarkers(){
		...
		if (startPoint != null){
			markerStart = updateItineraryMarker(null, startPoint, START_INDEX,
				osm.custommaps.R.string.departure, osm.custommaps.R.drawable.marker_departure, -1, null);
		}
		...
		if (destinationPoint != null){
			markerDestination = updateItineraryMarker(null, destinationPoint, DEST_INDEX,
				osm.custommaps.R.string.destination, osm.custommaps.R.drawable.marker_destination, -1, null);
		}
	}
    ...
    private void putRoadNodes(Road road){
		...
		Drawable icon = getResources().getDrawable(osm.custommaps.R.drawable.marker_node);
		int n = road.mNodes.size();
		MarkerInfoWindow infoWindow = new MarkerInfoWindow(org.osmdroid.bonuspack.R.layout.bonuspack_bubble, map);
		TypedArray iconIds = getResources().obtainTypedArray(osm.custommaps.R.array.direction_icons);
    	for (int i=0; i<n; i++){
    		RoadNode node = road.mNodes.get(i);
    		String instructions = (node.mInstructions==null ? "" : node.mInstructions);
    		Marker nodeMarker = new Marker(map);
    		nodeMarker.setTitle(getString(osm.custommaps.R.string.step)+ " " + (i+1));
    		nodeMarker.setSnippet(instructions);
    		nodeMarker.setSubDescription(Road.getLengthDurationText(node.mLength, node.mDuration));
    		nodeMarker.setPosition(node.mLocation);
    		nodeMarker.setIcon(icon);
    		nodeMarker.setInfoWindow(infoWindow); //use a shared infowindow.
    		int iconId = iconIds.getResourceId(node.mManeuverType, osm.custommaps.R.drawable.ic_empty);
    		if (iconId != osm.custommaps.R.drawable.ic_empty){
	...
	void updateUIWithRoads(Road[] roads){
		mRoadNodeMarkers.getItems().clear();
		//TextView textView = (TextView)findViewById(osm.custommaps.R.id.routeInfo);
		//textView.setText("");
		List<Overlay> mapOverlays = map.getOverlays();
		...
		for (int i=0; i<roads.length; i++) {
			...
			roadPolyline.setTitle(getString(osm.custommaps.R.string.route) + " - " + routeDesc);
		...
		if (pois != null){
			...
			for (POI poi:pois){
				...
				if (poi.mServiceId == POI.POI_SERVICE_NOMINATIM || poi.mServiceId == POI.POI_SERVICE_OVERPASS_API){
					icon = getResources().getDrawable(osm.custommaps.R.drawable.marker_poi);
					poiMarker.setAnchor(Marker.ANCHOR_CENTER, 1.0f);
				} else if (poi.mServiceId == POI.POI_SERVICE_GEONAMES_WIKIPEDIA){
					if (poi.mRank < 90)
						icon = getResources().getDrawable(osm.custommaps.R.drawable.marker_poi_wikipedia_16);
					else
						icon = getResources().getDrawable(osm.custommaps.R.drawable.marker_poi_wikipedia_32);
				} else if (poi.mServiceId == POI.POI_SERVICE_FLICKR){
					icon = getResources().getDrawable(osm.custommaps.R.drawable.marker_poi_flickr);
				} else if (poi.mServiceId == POI.POI_SERVICE_PICASA){
					icon = getResources().getDrawable(osm.custommaps.R.drawable.marker_poi_picasa_24);
	...
	String getOSMTag(String humanReadableFeature){
		HashMap<String,String> map = BonusPackHelper.parseStringMapResource(getApplicationContext(), osm.custommaps.R.array.osm_poi_tags);
	...
	void openLocalFileDialog(boolean open){
		...
		builder.setTitle(getString(osm.custommaps.R.string.file_kml_open));
		...
		builder.setPositiveButton(getString(osm.custommaps.R.string.ok), new DialogInterface.OnClickListener() {
		...
		builder.setNegativeButton(getString(osm.custommaps.R.string.cancel), new DialogInterface.OnClickListener() {
			@Override public void onClick(DialogInterface dialog, int which) {
				dialog.cancel();
			}
		});
	...
	void openUrlDialog(){
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setTitle(getString(osm.custommaps.R.string.url_kml_open));
		final EditText input = new EditText(this);
		input.setInputType(InputType.TYPE_CLASS_TEXT);
	...
	ProgressDialog createSpinningDialog(String title){
		...
		pd.setTitle(title);
		pd.setMessage(getString(osm.custommaps.R.string.wait));
	...
	void openFile(String uri, boolean onCreate, boolean isOverpassRequest){
		...
		new KmlLoadingTask(getString(osm.custommaps.R.string.loading)+" "+uri).execute(uri, onCreate, isOverpassRequest);
	}
	...
	Style buildDefaultStyle(){
		Drawable defaultKmlMarker = getResources().getDrawable(osm.custommaps.R.drawable.marker_kml_point);
	...
	@Override public void onCreateContextMenu(ContextMenu menu, View v,
		...
		//inflater.inflate(R.map_menu, menu);
	}

	@Override public boolean onContextItemSelected(MenuItem item) {
		switch (item.getItemId()) {
		case osm.custommaps.R.id.menu_departure:
			startPoint = new GeoPoint(mClickedGeoPoint);
			markerStart = updateItineraryMarker(markerStart, startPoint, START_INDEX,
				osm.custommaps.R.string.departure, osm.custommaps.R.drawable.marker_departure, -1, null);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_destination:
			destinationPoint = new GeoPoint(mClickedGeoPoint);
			markerDestination = updateItineraryMarker(markerDestination, destinationPoint, DEST_INDEX,
				osm.custommaps.R.string.destination, osm.custommaps.R.drawable.marker_destination, -1, null);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_viapoint:
			GeoPoint viaPoint = new GeoPoint(mClickedGeoPoint);
			addViaPoint(viaPoint);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_kmlpoint:
	...
	@Override public boolean onCreateOptionsMenu(Menu menu) {
		MenuInflater inflater = getMenuInflater();
		//inflater.inflate(osm.custommaps.R.menu.option_menu, menu);

		switch (mWhichRouteProvider){
		case OSRM:
			menu.findItem(osm.custommaps.R.id.menu_route_osrm).setChecked(true);
			break;
		case GRAPHHOPPER_FASTEST:
			menu.findItem(osm.custommaps.R.id.menu_route_graphhopper_fastest).setChecked(true);
			break;
		case GRAPHHOPPER_BICYCLE:
			menu.findItem(osm.custommaps.R.id.menu_route_graphhopper_bicycle).setChecked(true);
			break;
		case GRAPHHOPPER_PEDESTRIAN:
			menu.findItem(osm.custommaps.R.id.menu_route_graphhopper_pedestrian).setChecked(true);
			break;
		case GOOGLE_FASTEST:
			menu.findItem(osm.custommaps.R.id.menu_route_google).setChecked(true);
			break;
		}

		if (map.getTileProvider().getTileSource() == TileSourceFactory.MAPNIK)
			menu.findItem(osm.custommaps.R.id.menu_tile_mapnik).setChecked(true);
		else if (map.getTileProvider().getTileSource() == TileSourceFactory.MAPQUESTOSM)
			menu.findItem(osm.custommaps.R.id.menu_tile_mapquest_osm).setChecked(true);
		else if (map.getTileProvider().getTileSource() == MAPBOXSATELLITELABELLED)
			menu.findItem(osm.custommaps.R.id.menu_tile_mapbox_satellite).setChecked(true);
	...
	@Override public boolean onPrepareOptionsMenu(Menu menu) {
		if (mRoads != null && mRoads[mSelectedRoad].mNodes.size()>0)
			menu.findItem(osm.custommaps.R.id.menu_itinerary).setEnabled(true);
		else
			menu.findItem(osm.custommaps.R.id.menu_itinerary).setEnabled(false);
		if (mPOIs != null && mPOIs.size()>0)
			menu.findItem(osm.custommaps.R.id.menu_pois).setEnabled(true);
		else
			menu.findItem(osm.custommaps.R.id.menu_pois).setEnabled(false);
		return true;
	}
	...
	@Override public boolean onOptionsItemSelected(MenuItem item) {
		Intent myIntent;
		switch (item.getItemId()) {
		case osm.custommaps.R.id.menu_itinerary:
			...
			return true;
		case osm.custommaps.R.id.menu_pois:
			...
			return true;
		case osm.custommaps.R.id.menu_kml_url:
			openUrlDialog();
			return true;
		case osm.custommaps.R.id.menu_open_file:
			openLocalFileDialog(true);
			return true;
		case osm.custommaps.R.id.menu_overpass_api:
			openOverpassAPIWizard();
			return true;
		case osm.custommaps.R.id.menu_kml_get_overlays:
			insertOverlaysInKml();
			updateUIWithKml();
			return true;
		case osm.custommaps.R.id.menu_kml_tree:
			...
			return true;
		case osm.custommaps.R.id.menu_kml_styles:
			myIntent = new Intent(this, KmlStylesActivity.class);
			startActivityForResult(myIntent, KmlStylesActivity.KML_STYLES_REQUEST);
			return true;
		case osm.custommaps.R.id.menu_save_file:
			openLocalFileDialog(false);
			return true;
		case osm.custommaps.R.id.menu_kml_clear:
			mKmlDocument = new KmlDocument();
			updateUIWithKml();
			return true;
		case osm.custommaps.R.id.menu_route_osrm:
			mWhichRouteProvider = OSRM;
			item.setChecked(true);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_route_graphhopper_fastest:
			mWhichRouteProvider = GRAPHHOPPER_FASTEST;
			item.setChecked(true);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_route_graphhopper_bicycle:
			mWhichRouteProvider = GRAPHHOPPER_BICYCLE;
			item.setChecked(true);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_route_graphhopper_pedestrian:
			mWhichRouteProvider = GRAPHHOPPER_PEDESTRIAN;
			item.setChecked(true);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_route_google:
			mWhichRouteProvider = GOOGLE_FASTEST;
			item.setChecked(true);
			getRoadAsync();
			return true;
		case osm.custommaps.R.id.menu_tile_mapnik:
			setStdTileProvider();
			map.setTileSource(TileSourceFactory.MAPNIK);
			item.setChecked(true);
			return true;
		case osm.custommaps.R.id.menu_tile_mapquest_osm:
			setStdTileProvider();
			map.setTileSource(TileSourceFactory.MAPQUESTOSM);
			item.setChecked(true);
			return true;
		case osm.custommaps.R.id.menu_tile_mapbox_satellite:
			setStdTileProvider();
			map.setTileSource(MAPBOXSATELLITELABELLED);
			item.setChecked(true);
			return true;
		case osm.custommaps.R.id.menu_tile_mapsforge:
			...
			return true;
		case osm.custommaps.R.id.menu_download_view_area:{
		...
		case osm.custommaps.R.id.menu_clear_view_area:{
			CacheManager cacheManager = new CacheManager(map);
			int zoomMin = map.getZoomLevel();
		...
		case osm.custommaps.R.id.menu_cache_usage:{
			CacheManager cacheManager = new CacheManager(map);
