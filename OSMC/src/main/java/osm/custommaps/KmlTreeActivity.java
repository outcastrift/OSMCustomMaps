...
public class KmlTreeActivity extends Activity {
	...
	@Override protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(osm.custommaps.R.layout.kml_main);
		mListView = (ListView) findViewById(osm.custommaps.R.id.listviewKml);
		registerForContextMenu(mListView);

		mCurrentKmlFeature = osm.custommaps.MapActivity.mKmlStack.peek();
		mKmlClipboard = osm.custommaps.MapActivity.mKmlClipboard;

		eHeader = (EditText)findViewById(osm.custommaps.R.id.name);
		eHeader.setText(mCurrentKmlFeature.mName);

		eDescription = (EditText)findViewById(osm.custommaps.R.id.description);
		eDescription.setText(mCurrentKmlFeature.mDescription);

		sStyleSpinner = (Spinner) findViewById(osm.custommaps.R.id.styleSpinner);
		...
		CheckBox cVisible = (CheckBox)findViewById(osm.custommaps.R.id.checkbox_visible);
		...
		if (mCurrentKmlFeature instanceof KmlFolder){
			...
			mListView.setOnItemClickListener(new OnItemClickListener() {
				@Override public void onItemClick(AdapterView<?> arg0, View view, int position, long index) {
					...
					Intent myIntent = new Intent(view.getContext(), osm.custommaps.KmlTreeActivity.class);
					//myIntent.putExtra("KML", item);
					osm.custommaps.MapActivity.mKmlStack.push(item);
	...
	String[] stylesWithEmpty(){
		String[] styles = osm.custommaps.MapActivity.mKmlDocument.getStylesList();
	...
	public void onCheckboxClicked(View view) {
		...
		switch(view.getId()) {
		case osm.custommaps.R.id.checkbox_visible:
	...
	@Override public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		//inflater.inflate(osm.custommaps.R.menu.kml_item_menu, menu);
	}
	...
	@Override public boolean onContextItemSelected(MenuItem item) {
	    ...
	    switch (item.getItemId()) {
	        case osm.custommaps.R.id.kml_item_menu_cut: //=move to the emptied clipboard
	        ...
	        case osm.custommaps.R.id.kml_item_menu_behind:
	        	...
	        	return true;
	        case osm.custommaps.R.id.kml_item_menu_front:
	        	...
	        	return true;
	        case osm.custommaps.R.id.kml_item_menu_show_on_map:
	...
	@Override public boolean onCreateOptionsMenu(Menu menu) {
		MenuInflater inflater = getMenuInflater();
		//inflater.inflate(osm.custommaps.R.menu.kml_option_menu, menu);
		return true;
	}

	@Override public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
			case osm.custommaps.R.id.kml_option_menu_paste:
				if (mCurrentKmlFeature instanceof KmlFolder){
					KmlFolder currentKmlFolder = (KmlFolder)mCurrentKmlFeature;
				...
				return true;
			case osm.custommaps.R.id.kml_option_menu_new:
				if (mCurrentKmlFeature instanceof KmlFolder){
					KmlFolder currentKmlFolder = (KmlFolder)mCurrentKmlFeature;
