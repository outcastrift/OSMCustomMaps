...
public class WarpedImageOverlay extends Overlay {
  ...
  /*public void WARPED(){
    ...
    /* public boolean draw(Canvas canvas, MapView mapView, boolean shadow, long when) {//TODO Original Code
       ...
       GeoPoint mapCenter = (GeoPoint) mapView.getMapCenter();  //TODO Changed to Cast to GEPOINT
       ...
       if (zoom != lastDrawnZoom || !mapCenter.equals(lastDrawnLocation)) {
         ...
         lastDrawnZoom = zoom;
         lastDrawnLocation = (GeoPoint) mapCenter;  //TODO Changed to Cast to GeoPoint
       }
    ...
    public Matrix computeImageToGeoMatrix(MapView map) {
        ...
        for (int i = 0; i < 8; i += 2) {
            ...
            GeoPoint gp = (GeoPoint) map.getProjection().fromPixels(x, y); //TODO casted to geopooint
