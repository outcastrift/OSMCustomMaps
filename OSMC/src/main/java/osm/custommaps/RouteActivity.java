...
public class RouteActivity extends Activity {
	...
	@Override public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(osm.custommaps.R.layout.items_list);

        ListView list = (ListView) findViewById(osm.custommaps.R.id.items);
        ...
        osm.custommaps.RoadNodesAdapter adapter = new osm.custommaps.RoadNodesAdapter(this, road);
...
class RoadNodesAdapter extends BaseAdapter implements OnClickListener {
    ...
    public RoadNodesAdapter(Context context, Road road) {
        mContext = context;
        mRoad = road;
		iconIds = mContext.getResources().obtainTypedArray(osm.custommaps.R.array.direction_icons);
    }
    ...
    @Override public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (convertView == null) {
            ...
            convertView = inflater.inflate(osm.custommaps.R.layout.item_layout, null);
        }
        TextView tvTitle = (TextView)convertView.findViewById(osm.custommaps.R.id.title);
        String instructions = (entry.mInstructions==null ? "" : entry.mInstructions);
        tvTitle.setText("" + (position+1) + ". " + instructions);
        TextView tvDetails = (TextView)convertView.findViewById(osm.custommaps.R.id.details);
        tvDetails.setText(Road.getLengthDurationText(entry.mLength, entry.mDuration));
		int iconId = iconIds.getResourceId(entry.mManeuverType, osm.custommaps.R.drawable.ic_empty);
   		Drawable icon = mContext.getResources().getDrawable(iconId);
		ImageView ivManeuver = (ImageView)convertView.findViewById(osm.custommaps.R.id.thumbnail);
