...
public class KmlStyleActivity extends Activity {
	...
	@Override protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(osm.custommaps.R.layout.kml_style);
		...
		eStyleId = (EditText)findViewById(osm.custommaps.R.id.style_id);
		mInitialStyleId = getIntent().getExtras().getString("STYLE_ID");
		eStyleId.setText(mInitialStyleId);

		eIconHref = (EditText)findViewById(osm.custommaps.R.id.icon);
		if (mStyle.mIconStyle.mHref != null)
			eIconHref.setText(mStyle.mIconStyle.mHref);

		eOutlineColor = (EditText)findViewById(osm.custommaps.R.id.outlineColor);
		eOutlineColor.setText(mStyle.mLineStyle.colorAsAndroidString());

		tOutlineWidthValue = (TextView)findViewById(osm.custommaps.R.id.outlineWidthValue);
		tOutlineWidthValue.setText(""+(int)mStyle.mLineStyle.mWidth);

		sbOutlineWidth = (SeekBar)findViewById(osm.custommaps.R.id.outlineWidth);
		...
		eFillColor = (EditText)findViewById(osm.custommaps.R.id.fillColor);
		eFillColor.setText(mStyle.mPolyStyle.colorAsAndroidString());

		Button btnOk = (Button) findViewById(osm.custommaps.R.id.btnOK);
	...
	protected void save(){
		...
		HashMap<String, StyleSelector> styles = osm.custommaps.MapActivity.mKmlDocument.getStyles();
